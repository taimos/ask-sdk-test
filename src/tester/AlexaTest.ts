...
// const upsServiceMock: { getProfileName: () => string | undefined; getProfileGivenName: () => string | undefined; getProfileEmail: () => string | undefined; getProfileMobileNumber: () => string | undefined; } = {
...
export class AlexaTest {
  ...
  private dynamoDBTable: string | undefined;
  private partitionKeyName: string | undefined;
  private attributesName: string | undefined;
  ...
  private async invokeLambdaCloudformation(
    stackName: string,
    logicalName: string,
    settings: TestSettings,
    currentItem: SequenceItem,
    request: RequestEnvelope): Promise<any> {
  ...
  private async invokeLambda(name: string, _settings: TestSettings, currentItem: SequenceItem, request: RequestEnvelope): Promise<any> {
  ...
  private mockDynamoDB(settings: TestSettings, currentItem: SequenceItem): void {
    if (this.dynamoDBTable) {
        ...
        if (storesAttributes) {
          for (const att in storesAttributes) {
            if (storesAttributes.hasOwnProperty(att)) {
              ...
              const expectedAttr: any = storesAttributes[att];
      ...
      dynamoDBMock.getMock = (params: GetItemInput, callback) => {
        ...
        if (!this.partitionKeyName || !this.attributesName) {
          fail();
          return;
        }
