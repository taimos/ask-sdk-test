...
export class SpeechValidator extends ResponseValidator {
  ...
  public validate(currentItem: SequenceItem, response: ResponseEnvelope): void {
    let actualSay: string | undefined;
    ...
    if (currentItem.saysLike !== undefined) {
      expect(actualSay && actualSay.indexOf(currentItem.saysLike) >= 0, `Speech did not contain specified text. Expected ${actualSay} to be like ${currentItem.saysLike}`).to.be.true;
    }
    if (currentItem.saysNothing) {
      expect(actualSay, 'Should have said nothing').to.be.undefined;
    }

    let actualReprompt: string | undefined;
    ...
    if (currentItem.repromptsLike !== undefined) {
      expect(actualReprompt && actualReprompt.indexOf(currentItem.repromptsLike) >= 0, `Reprompt did not contain specified text. Expected ${actualReprompt} to be like ${currentItem.repromptsLike}`).to.be.true;
    }
